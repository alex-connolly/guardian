...
Obviously, there are two ways of optimizing such a loop. One could use the knowledge that the value of ```count``` is ultimately immaterial to optimize the entire loop away. However, as this is only observable after the bytecode would have been generated, it necessitates either:
